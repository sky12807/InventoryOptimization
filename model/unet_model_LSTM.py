...
#         print(f'Metrod shape: {metrod.shape}')
#         print(f'Grid shape: {grid.shape}')
#         print(f'All Label shape: {all_label.shape}')
        # Batch Normalization for Metrod
        B, T, C, H, W = metrod.shape
        metrod = metrod.reshape(B*T,C,H,W)
        metrod = self.bn_metrod(metrod)
        metrod = metrod.reshape(B,T,C,H,W)
#         print(f'Metrod shape: {metrod.shape}')
        
        B, T, C, H, W = all_label.shape
        y = all_label.reshape(B*T,C,H,W)
        y = self.bn_y(y)
        y = y.reshape(B,T,C,H,W)
#         print(f'Y shape: {y.shape}')
        
        grid = self.bn_grid(grid)
        
        # Concat y and metrod
        y_metrod = torch.cat([y, metrod],dim = 2)
